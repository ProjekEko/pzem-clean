...
#define FIREBASE_HOST "pzemm-6b93a-default-rtdb.firebaseio.com"
#define FIREBASE_AUTH "AIzaSyDPp_SyvQJApKckpT-YpGBbfMMBAkpvj7A"
...
void setup() {
  ...
  // config.signer.tokens.oauth2.access_token = FIREBASE_AUTH; // alternatif
  ...
  if (Firebase.ready()) {
    ...
    Serial.println("\n❌ Firebase Gagal! Cek API Key & URL");
    Serial.println("   Pastikan Rules database: .read=true .write=true");
  }
  ...
  // ============ SET DATA AWAL ============
  if (Firebase.ready()) {
    if (Firebase.RTDB.setString(&fbdo, "/sistem/status", "ONLINE")) {
      Serial.println("✅ Data awal terkirim!");
    } else {
      Serial.println("❌ Gagal kirim data awal!");
    }
  }
